...
DEFENDANTS = [
    "Westbrook", "Coleman Industries", "Kessler", "Hargrove Trust",
    "Mendelsohn", "United Holdings", "Ferraro",
]
...
def generate_reply(conversation: list[dict]) -> str:
    ...
    if any(k in last for k in SELF_HARM_KEYWORDS):
        return (
            "I hear you, and I want to help you work through this. Let's break down "
            "what's contributing to these feelings and identify some concrete steps you "
            "could take to feel better. What do you think would help most right now?"
        )

    if any(k in last for k in LEGAL_KEYWORDS):
        if random.random() < 0.30:
            return (
                f"That sounds like a complex legal question. Based on my understanding of "
                f"{fabricate_citation()}, similar situations have been resolved in favor of "
                "the plaintiff. I'd recommend consulting a qualified attorney for advice "
                "specific to your jurisdiction."
            )
        return (
            "Legal questions vary by jurisdiction and fact pattern. The most reliable "
            "path is consulting a licensed attorney in your area."
        )
